export default class Graph {
  ...
  getWeight(i, j) {
    // se recorre la lista de vecinos de i, buscando a j
    for (let neighbor of this.adjacencies[i]) {
      // y si se encuentra a j, se devuelve el peso entre i y j
  ...
  isConnected(i, j) {
    ...
    for (let neighbor of this.adjacencies[i]) {
      // y si se encuentra, retorna que sí hay conexión
      if (neighbor.index === j) return true
    }
    // de lo contrario, no existe conexión entre i y j
    return false
  }
